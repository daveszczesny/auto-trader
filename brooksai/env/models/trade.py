...
from env.models.constants import TradeType, DEFAULT_TRADE_TTL, Environments, ENVIRONMENT, CONTRACT_SIZE, LEVERAGE
from env.utils.converter import pip_to_profit, pips_to_price_chart
...
class Trade:
    ...
    def __init__(self, lot_size: float, open_price: float, trade_type: TradeType, stop_loss: Optional[float] = None, take_profit: Optional[float] = None) :
        self.uuid = str(uuid1())
        ...
        if ENVIRONMENT == Environments.DEV:
            with open('brooksai_logs.txt', 'a') as f:
                f.write(f"Trade {self.uuid} opened with lot size: {lot_size}, open price: {open_price}, trade type: {trade_type}, SL: {self.stop_loss}, TP: {self.take_profit}\n")
...
def get_trade_state(id: str, current_price: float):
...
def trigger_stop_or_take_profit(current_price: float) -> float:
    ...
    for trade in open_trades:
        ...
        if trade.trade_type == TradeType.LONG:
            ...
            take_profit_price = trade.open_price + pips_to_price_chart(trade.take_profit) if trade.take_profit else None
            stop_loss_price = trade.open_price - pips_to_price_chart(trade.stop_loss) if trade.stop_loss else None
...
def open_trade(trade: Trade):
    ...
    if ENVIRONMENT == Environments.PROD:
        ...
        # TODO: Send request to broker to open trade
